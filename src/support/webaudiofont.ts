// Original code: https://github.com/surikov/webaudiofont
// Converted to somewhat proper async/await and TypeScript by me
...
export interface WaveEnvelope extends GainNode {
...
export interface WaveSlide {
    when: number;
    delta: number;
}

export interface WaveAHDSR {
    duration: number;
    volume: number;
}
...
class WebAudioFontLoader {
	...
	async getPreset(audioContext: AudioContext, presetInfo: PresetInfo) : Promise<WavePreset> {
		await this.loadIfNotLoaded(audioContext, presetInfo.url, presetInfo.variable);
		return window[presetInfo.variable] as WavePreset;
	}

	private async loadIfNotLoaded(audioContext: AudioContext, filePath: string, variableName: string) {
		if (window[variableName]) {
			if (this.loading.has(variableName)) {
				await this.waitForCondition(() => !this.loading.has(variableName));
			}
			return;
		}

		this.loading.add(variableName);
		...
		await this.waitForCondition(() => window[variableName] != undefined);
		await this.player.adjustPreset(audioContext, (window[variableName] as any) as WavePreset);

		this.loading.delete(variableName);
	}

	private async waitForCondition(condition: () => boolean) {
		while (!condition()) {
	...
	loaded(variableName: string): boolean {
        ...
        const preset = window[variableName] as WavePreset;
	...
	waitLoad(onFinish: () => void) {
        if (this.progress() >= 1) {
			...
			setTimeout(() => this.waitLoad(onFinish), 100);
		}
	};

	private instrumentTitles(): string[] {
    ...
    instrumentInfo(n: number): PresetInfo {
        const key: string = this.instrumentKeys()[n];
        const p = parseInt(key.substring(0, 3));
        return {
			variable: '_tone_' + key,
    ...
    findInstrument(program: number): number {
		...
		for (let i = 0; i < this.instrumentKeys().length; i++) {
			if (program == parseInt(this.instrumentKeys()[i].substring(0, 3))) {
				return i;
			}
		}
    ...
    drumInfo(n: number): PresetInfo {
        ...
        const p = parseInt(key.substring(0, 2));
        return {
			variable: '_drum_' + key,
    ...
    findDrum(nn: number): number {
		for (let i = 0; i < this.drumKeys().length; i++) {
			if (nn == parseInt(this.drumKeys()[i].substring(0, 2))) {
				return i;
			}
		}
		return 0;
...
export class WebAudioFontPlayer {
    ...
    private limitVolume(volume: number | undefined): number {
        if (!volume) {
            volume = 0.5;
        }
        return volume;
    ...
    async queueChord(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[],
					 duration: number, volume?: number, slides?: WaveSlide[][]): Promise<WaveEnvelope[]> {
        ...
        for (let i = 0; i < pitches.length; i++) {
            ...
            const envlp = await this.queueWaveTable(audioContext, target, preset, when, pitches[i],
	...
	async queueStrumUp(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[],
					   duration: number, volume?: number, slides?: WaveSlide[][]): Promise<WaveEnvelope[]> {
        pitches.sort(function (a, b) {
            return b - a;
        });
        return await this.queueStrum(audioContext, target, preset, when, pitches, duration, volume, slides);
    };

	async queueStrumDown(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[],
						 duration: number, volume?: number, slides?: WaveSlide[][]): Promise<WaveEnvelope[]> {
        pitches.sort(function (a, b) {
            return a - b;
        });
    ...
    async queueStrum(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[],
					 duration: number, volume?: number, slides?: WaveSlide[][]): Promise<WaveEnvelope[]> {
        volume = this.limitVolume(volume);
        ...
        for (let i = 0; i < pitches.length; i++) {
            ...
            const envlp = await this.queueWaveTable(audioContext, target, preset, when + i * 0.01, pitches[i],
				duration, volume - Math.random() * 0.01, singleSlide);
            if (envlp) envelopes.push(envlp);
            volume = 0.9 * volume;
        }
    ...
    async queueSnap(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[],
			  duration: number, volume?: number, slides?: WaveSlide[][]): Promise<WaveEnvelope[]> {
        volume = this.limitVolume(volume);
    ...
    private resumeContext(audioContext: AudioContext): Promise<void> {
        try {
    ...
    async queueWaveTable(audioContext: AudioContext, target: AudioNode, preset: WavePreset, when: number, pitch: number,
						 duration: number, volume?: number, slides?: WaveSlide[]): Promise<WaveEnvelope> {

		volume = this.limitVolume(volume);

        const zone = await this.findZone(audioContext, preset, pitch);
		if (!zone || !zone.buffer) {
			throw new Error('empty buffer');
		}

		await this.resumeContext(audioContext);
		...
		if (startWhen < audioContext.currentTime) {
			startWhen = audioContext.currentTime + when;
		}
		...
		if (slides && slides.length > 0) {
			...
			for (let i = 0; i < slides.length; i++) {
				...
				const newPlaybackRate = Math.pow(2, (100.0 * nextPitch - baseDetune) / 1200.0);
    ...
    setupEnvelope(audioContext: AudioContext, envelope: WaveEnvelope, zone: WaveZone, volume: number, when: number, sampleDuration: number, noteDuration: number) {
        (envelope as GainNode).gain.setValueAtTime(this.noZeroVolume(0), audioContext.currentTime);
    ...
    findEnvelope(audioContext: AudioContext, target: AudioNode): WaveEnvelope {
        ...
        if (!envelope) {
            ...
            (envelope as GainNode).connect(target);
            envelope.cancel = function () {
                if (envelope && (envelope.when + envelope.duration > audioContext.currentTime)) {
                    (envelope as GainNode).gain.cancelScheduledValues(0);
                    (envelope as GainNode).gain.setTargetAtTime(0.00001, audioContext.currentTime, 0.1);
    ...
    private async adjustZone(audioContext: AudioContext, zone: WaveZone) {
        if (zone.buffer) {
            //
        } else {
            zone.delay = 0;
            if (zone.sample) {
                ...
                if (zone.file) {
                    ...
                    await audioContext.decodeAudioData(arraybuffer, function (audioBuffer) {
    ...
    async findZone(audioContext: AudioContext, preset: WavePreset, pitch: number): Promise<WaveZone | null> {
        ...
        try {
            if (zone) await this.adjustZone(audioContext, zone);
        } catch (ex) {
